...
export function ContractList({ contracts }: { contracts: ContractListItem[] }) {
  return (
    <div className="grid gap-4">
        ...
        <Card key={contract.id}>
          <CardContent className="flex flex-col gap-4 pt-6 lg:flex-row lg:items-center lg:justify-between">
            <div className="space-y-2">
              ...
              <p className="max-w-2xl text-sm text-slate-500 dark:text-zinc-400">{contract.description || "No description provided yet."}</p>
      ...
      {contracts.length === 0 ? (
        <Card>
          <CardContent className="py-12 text-center text-sm text-slate-500 dark:text-zinc-400">No contracts found for this filter yet.</CardContent>
        </Card>
      ) : null}
    </div>
  );
