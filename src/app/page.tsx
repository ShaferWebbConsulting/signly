...
export default function Home() {
  return (
    <main className="min-h-screen bg-gradient-to-b from-slate-50 via-white to-blue-50 text-slate-950 dark:from-zinc-950 dark:via-zinc-950 dark:to-blue-950/40 dark:text-white">
      <section className="mx-auto flex max-w-6xl flex-col gap-12 px-6 py-24 lg:flex-row lg:items-center lg:justify-between">
        <div className="max-w-2xl space-y-8">
          <span className="inline-flex items-center gap-2 rounded-full border border-slate-200 bg-white px-4 py-2 text-sm text-slate-600 dark:border-white/10 dark:bg-white/5 dark:text-zinc-300">
            <Sparkles className="h-4 w-4 text-blue-500 dark:text-blue-400" /> Contract operations for modern teams
          </span>
          <div className="space-y-6">
            <h1 className="text-5xl font-semibold tracking-tight sm:text-6xl">Create, Sign &amp; Manage Contracts Online</h1>
            <p className="max-w-xl text-lg text-slate-600 dark:text-zinc-300">Signly helps legal, sales, and operations teams draft faster, collect signatures securely, and verify every finalized agreement with a complete audit trail.</p>
          </div>
        ...
        <Card className="w-full max-w-xl border-slate-200 bg-white text-slate-950 shadow-2xl shadow-blue-500/10 dark:border-white/10 dark:bg-white/5 dark:text-white">
          <CardContent className="grid gap-6 pt-6">
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <div className="rounded-2xl border border-slate-200 bg-slate-50 p-5 dark:border-white/10 dark:bg-zinc-900/80">
          ...
          return (
            <Card key={feature.title} className="border-slate-200 bg-white text-slate-950 dark:border-white/10 dark:bg-white/5 dark:text-white">
              <CardContent className="space-y-4 pt-6">
                <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-blue-100 text-blue-600 dark:bg-blue-500/15 dark:text-blue-300"><Icon className="h-6 w-6" /></div>
                <h2 className="text-xl font-semibold">{feature.title}</h2>
                <p className="text-slate-600 dark:text-zinc-300">{feature.description}</p>
              </CardContent>
            </Card>
          );
        })}
      ...
      <section className="mx-auto max-w-6xl px-6 py-16">
        <div className="mb-10 max-w-2xl">
          ...
          <p className="mt-3 text-slate-600 dark:text-zinc-300">Start with secure e-signatures, then unlock deeper automation, audit trails, and enterprise controls as you grow.</p>
        </div>
